...
const menuItems: NavItem[] = [
  { name: ct('nav.overview'), icon: LayoutDashboard, href: '/dashboard' },
  { name: ct('nav.countries'), icon: Globe, href: '/dashboard/countries' },
  { name: ct('nav.platformAdmins'), icon: Users, href: '/dashboard/admins' },
  { name: ct('nav.groups'), icon: ShieldCheck, href: '/dashboard/groups' },
  { name: ct('nav.membersKyc'), icon: UserCheck, href: '/dashboard/users' },
  { name: ct('nav.loans'), icon: Banknote, href: '/dashboard/loans' },
  { name: ct('nav.providers'), icon: Database, href: '/dashboard/payments' },
  ...
  { name: ct('nav.settings'), icon: Settings, href: '/dashboard/settings' },
]
    ...
    <aside className="dashboard-shell flex h-full w-[260px] flex-shrink-0 flex-col border-r border-gray-200 bg-white px-5 text-gray-900 dark:border-gray-800 dark:bg-gray-900 dark:text-white">
      <div className="flex-shrink-0 py-8">
        <Link href="/dashboard" className="flex items-center gap-3">
          ...
          <span className="text-xl font-semibold tracking-tight text-gray-900 dark:text-white">{ct('shell.title')}</span>
        </Link>
      </div>
      ...
      <div className="flex-shrink-0 py-4">
        <div className="mb-3 flex items-center gap-3 rounded-lg bg-gray-50 px-3 py-2.5 dark:bg-gray-800">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-gray-800 dark:text-white">{user?.full_name || ct('shell.userFallback')}</p>
