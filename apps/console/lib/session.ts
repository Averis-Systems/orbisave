/**
 * Server-side session claims for the Console.
 *
 ...
 * a domain-scoped cookie the Console will receive (cookie tossing). Reading an
 * unverified role claim would let that forged cookie choose its own role.
 * Verification is what makes the claim trustworthy.
 */
import { jwtVerify, importSPKI, type JWTPayload } from 'jose'

export const ACCESS_COOKIE = 'orbi_console_access'
export const REFRESH_COOKIE = 'orbi_console_refresh'

/** Roles permitted to hold a Console session. */
export const PORTAL_ROLES = ['super_admin'] as const
